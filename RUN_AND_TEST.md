...
3. Inbound: Twilio must reach the API over public HTTPS. In production that's the stable Railway URL — point the number's messaging webhook at `https://api-production-f9f2.up.railway.app/twilio/sms` (Console → Phone Numbers → Active numbers → the number → Messaging → "A message comes in"); `TWILIO_WEBHOOK_BASE_URL` is already set to it on Railway. For a **local** API instead, expose it with a tunnel (`cloudflared tunnel --url http://localhost:4100`), set `TWILIO_WEBHOOK_BASE_URL=<tunnel URL>` in `.env`, restart, and point the webhook there. Replies thread into workflows exactly like console replies; status callbacks reconcile via `<base>/twilio/status` automatically when the base URL is https.
...
| Local login says **Invalid credentials** for known-good `dental-demo` (or API responses look odd, e.g. `{"code":"not_found"}`) | another project's API is squatting on **port 4100** — the login is hitting the wrong backend. `lsof -nP -iTCP:4100 -sTCP:LISTEN` → if the command's cwd isn't this repo, kill it and restart `pnpm --filter @dental/api dev`. Other projects on this machine (Caseline, Healthcare Notes) default to the same 3000/4100 ports. (The hosted site is immune — it talks to Railway, not localhost.) |
